...
--1.WRITE A QUERY TO SELECT ALL DATA FROM CUSTOMERS  TABLE
...
-- 3. Write a query to select the product name and 
--its price from the `Products` table where the price is greater than 500.
...
select*from sales;
select*from customers;
-- 14. Write a query to join the `Sales` and `Products` tables on product_id, and 
--select the product name and total sales amount.
...
select first_name,last_name,SUM(total_amount) as total_amount_spent
from customers c 
JOIN assignment.sales s ON c.customer_id = s.customer_id
...
membership_status
from customers 
join customers  on membership_status=c.customer_name and 


-- 18. Write a query to join the `Sales` and `Products` tables, and 
...
-- 21. Write a query to select customers who have purchased products that are either
-- in the 'Electronics' or 'Appliances' category.
...
-- 23. Write a query to join the `Sales` table with the `Customers` table and 
--select customers who made a purchase in the year 2023.
select first_name,last_name,sale_date
from customers c
JOIN assignment.sales s ON c.customer_id = s.customer_id
...
select count (DISTINCT c.customer_id) as customers_over_500
from customers c
join sales s on c.customer_id = s.customer_id
...
select count(*) as total_gold_sales
from assignment.sales s 
...
-- 28. Write a query to join the `Products` and `Inventory` tables and 
--find all products that have low stock (less than 10).
...
select *from assignment.inventory ;
-- 29. Write a query to find customers who have purchased more than 5 products and 
--show the total quantity of products they have bought.
...
-- 32. Write a query to find the total amount spent by each customer in 2023 and 
--list the customers in descending order.
...
select p.product_id,p.product_name,SUM(total_amount)as total_sales
from products p
join sales s on p.product_id=s.product_id 
...
select c.customer_id,c.first_name,c.last_name,c.registration_date,s.product_id,s.sale_date,s.quantity_sold
from customers c
join sales s on c.customer_id=s.customer_id 
...
-- 36. Write a query to join the `Sales` table with the `Products` table and
-- filter the results by products priced between 100 and 500.
...
-- 37. Write a query to find the most frequent customer who made purchases from the `Sales` table.
 select c.customer_id,c.first_name,c.last_name,
sum(s.sale_id) as total_purchases
from sales s
join customers c on s.customer_id = c.customer_id
group by c.customer_id, c.first_name, c.last_name
...
select customer_id, sum(quantity_sold ) as total_quantity_sold
from Sales
group by customer_id;


-- 39. Write a query to find the products with the highest stock and
--lowest stock, and display them together in a single result set.
select p.product_name ,stock_quantity,'highest stock' as stock_type
from products p 
...
select p.product_name,extract(year from s.sale_date) as sale_year,
extract(month from s.sale_date) as sale_month, sum(s.quantity_sold * p.price) as total_sales
from Products p
join Sales s on p.product_id = s.product_id
...
select i.product_id, sum(s.quantity_sold ) as total_sold,i.stock_quantity
from Sales s
...
select c.customer_id,c.first_name,c.last_name,sum(s.quantity_sold * p.price) as total_spent
from Customers c
join Sales s on c.customer_id = s.customer_id
join Products p
on s.product_id = p.product_id
...
-- 48. Write a query to select the products with a price range between $200 and $800, and
...
select p.product_id,p.product_name,p.price,sum(s.quantity_sold ) as total_quantity_sold
from Products p
join Sales s on p.product_id = s.product_id
where p.price > 200
